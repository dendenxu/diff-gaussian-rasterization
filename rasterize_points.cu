...
//   auto int_opts = means3D.options().dtype(torch::kInt32);
//   auto float_opts = means3D.options().dtype(torch::kFloat16);
...
// 	  rendered = CudaRasterizer::Rasterizer::forward_half(
// 	    geomFunc,
// 		binningFunc,
// 		imgFunc,
// 	    P, degree, M,
// 		(const __half *)background.contiguous().data_ptr<at::Half>(),
// 		W, H,
// 		(const __half *)means3D.contiguous().data_ptr<at::Half>(),
// 		(const __half *)sh.contiguous().data_ptr<at::Half>(),
// 		(const __half *)colors.contiguous().data_ptr<at::Half>(), 
// 		(const __half *)opacity.contiguous().data_ptr<at::Half>(), 
// 		(const __half *)scales.contiguous().data_ptr<at::Half>(),
// 		scale_modifier,
// 		(const __half *)rotations.contiguous().data_ptr<at::Half>(),
// 		(const __half *)cov3D_precomp.contiguous().data_ptr<at::Half>(), 
// 		(const __half *)viewmatrix.contiguous().data_ptr<at::Half>(), 
// 		(const __half *)projmatrix.contiguous().data_ptr<at::Half>(),
// 		(const __half *)campos.contiguous().data_ptr<at::Half>(),
// 		tan_fovx,
// 		tan_fovy,
// 		prefiltered,
// 		(__half *)out_color.contiguous().data_ptr<at::Half>(),
// 		(__half *)out_depth.contiguous().data_ptr<at::Half>(),
// 		(__half *)out_alpha.contiguous().data_ptr<at::Half>(),
// 		radii.contiguous().data_ptr<int>(),
// 		debug);
//   }
//   return std::make_tuple(rendered, out_color, out_depth, out_alpha, radii, geomBuffer, binningBuffer, imgBuffer);
