...
std::tuple<torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor> fusedPreprocess4DbetaSparse(
	const torch::Tensor& means3D,
	const torch::Tensor& cov,
	const torch::Tensor& ms,
	const torch::Tensor& cov_t,
...
{ 
  ...
  if(P != 0)
  {
	CudaRasterizer::Rasterizer::fusedPreprocess4DbetaSparse(P, deg, deg_t, M,
		means3D.contiguous().data_ptr<float>(),
		cov.contiguous().data_ptr<float>(),
		ms.contiguous().data_ptr<float>(),
		cov_t.contiguous().data_ptr<float>(),
